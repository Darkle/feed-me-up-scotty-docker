...
- If you would like to be notified if the container exits, you can comment out the current `CMD` and uncomment out the `CMD` below. Then run the `docker run` or `podman run` with the extra arg of `-e HEALTHCHECK_IO_URL=https://foo` with `https://foo` being the https://healthchecks.io check URL.
